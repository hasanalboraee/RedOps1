...
func (h *TaskHandler) GetTasksByOperation(c *gin.Context) {
	...
	tasks, err := h.repo.GetByOperationID(objectID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *TaskHandler) GetTasksByAssignedUser(c *gin.Context) {
	...
	tasks, err := h.repo.GetByAssignedUser(objectID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *TaskHandler) UpdateTaskStatus(c *gin.Context) {
	...
	if err := h.repo.UpdateStatus(objectID, status); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *TaskHandler) UpdateTaskResults(c *gin.Context) {
	id := c.Param("id")
	objectID, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ID format"})
	...
	if err := h.repo.UpdateResults(objectID, results); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Task results updated successfully"})
}
